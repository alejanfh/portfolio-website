...
export function Project({ title, description, tags, imageUrl }: ProjectProps) {
  ...
  const { scrollYProgress } = useScroll({
    target: ref,
    // start, when the bottom of the viewport crosses the top of the target
    // stop, when the bottom of the viewport has gone 33% beyond the end of the project
    offset: ['0 1', '1.33 1'],
  })
  ...
  return (
    ...
    >
      <section className='group-odd:pl-8 bg-gray-100 max-w-[42rem] rounded-lg border border-black/5 overflow-hidden sm:pr-8 relative sm:h-[20rem]  hover:bg-gray-200 transition'>
